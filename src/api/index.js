	...
	Object.keys(genericApi).map((key) => {
		...
		if (genericApi[key].indexingConfig && scope.searchIndexer) {
			if (genericApi[key].indexingConfig instanceof Function) {
				genericApi[key].indexingConfig = genericApi[key].indexingConfig({})
			}
			const { searchIndexer } = scope.config || {}
			scope.searchIndexer.createIndex((searchIndexer.indexPrefix || '') + key, {
...
export function getMethodSecurity(key, method) {
	if (!securityMap[key] || typeof securityMap[key][method] == 'undefined') {
		return undefined
	}
	return securityMap[key][method]
}
